...
bot.on('callbackQuery', (msg) => {
	...
	if(AdminJson[keyID].includes(msg.from.id)){
		...
		if(data[0] === "nP")
		{
			...
			Promise.all([bot.sendMessage(process.env.Telegram_EBGKanal_ID, msg.message.text, { parseMode: 'html' , webPreview: false}), Sender.pushDiscord(msg.message.text)]).then((data) => {
